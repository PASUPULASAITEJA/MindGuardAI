...
| **University Student PHQ-GAD Cohort** | Multi-instrument clinical risk model training (`scripts/train_phq_gad.py`). | 24,292 university students (92% undergraduate, mean age 20.6) with full item responses for PHQ-9, GAD-7, ISI (Insomnia), and PSS (Perceived Stress) alongside item hesitation latency. Model achieves 99.2% accuracy and 98.2% high-risk recall (`backend/app/ml/models/phq_gad_risk_model.joblib`). |
