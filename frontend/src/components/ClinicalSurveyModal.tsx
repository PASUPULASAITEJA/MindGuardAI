...
import { X, ChevronLeft, ChevronRight, CheckCircle2, ClipboardCheck, ShieldCheck } from "lucide-react";
...
export const ClinicalSurveyModal: React.FC<ClinicalSurveyModalProps> = ({
  ...
  const surveyTitle = surveyType === "phq-9" ? "PHQ-9 Depression Screener" : "GAD-7 Anxiety Screener";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: "Clinical Screener Recorded",
        description: `${surveyTitle} submitted. Clinical severity: ${res.data?.severity || "Analyzed"}.`,
        variant: "success"
      });
