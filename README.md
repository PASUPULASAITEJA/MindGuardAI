...
```bash
# 1. Full System Integration Suite (32/32 tests)
...
# 4. Backend Unit & Integration Suite (39/39 tests)
cd backend && .\.venv\Scripts\python -m pytest tests/

# 5. Frontend Type-Check & Production Build Validation
cd frontend && npm run build
```

---

## 6. Machine Learning Pipeline & Risk Stratification

The ML pipeline is partitioned into two distinct engines to perform comprehensive NLP classification, longitudinal tracking, and structural clinical risk assessment.

```mermaid
graph TD
    %% Input
    Input["Raw Text Journal Entry / Check-In"] --> PII["PII Masking Engine - NER Regex"]
    ...
    subgraph RiskAssessmentEngine ["Risk Assessment Engine"]
        Emotions --> FeatureEng["Feature Engineering <br> Volatility, exam index, sleep rest, rolling avg"]
    ...
    DecisionDiamond -->|Low (Score >= 65)| SelfHelp["Self-help Resources & Positive Reinforcement"]
    DecisionDiamond -->|Medium (Score 35-64)| Coping["Guided Coping & In-Chat CBT Tools"]
    DecisionDiamond -->|High / Red (Score < 35 or Trigger)| Warning["Emergency SOS & Counselor Alert Queue"]
```

### 6.1 Multi-Modal Risk Stratification Criteria
...
### 6.2 Data Preprocessing & PII Masking
